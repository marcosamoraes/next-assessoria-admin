...
type PartnerStoreLogoProps = {
  partnerStore: IPartnerStore
  onChange: (url: string) => void
}
export default function PartnerStoreLogo({ partnerStore, onChange }: PartnerStoreLogoProps) {
  const [tempFile, setTempFile] = useState<string | null>(null)
  const [updated, setUpdated] = useState<boolean>(false)

  useEffect(() => {
    if (partnerStore.id && partnerStore.logo && !updated) {
      setTempFile(partnerStore.logo)
    }
  }, [partnerStore, updated])
  ...
  const handleFileChange = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    const { data: url }: any = await Upload.post('partner-stores', { file: files[0] })
  ...
  return (
    <div className="mb-4">
      <h3 className="w-full text-md font-light text-primary mb-2">Logo</h3>
