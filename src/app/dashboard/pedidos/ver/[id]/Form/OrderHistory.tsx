...
export default function OrderHistory({ order }: { order: IOrder | null }) {
  ...
  return (
    <div className="mb-4">
      ...
      <div className="bg-white px-10 py-7 rounded-xl flex flex-col">
        <div className="flex flex-col flex-1">
          <div className="flex flex-row justify-between">
            <p className="text-md text-gray-700 text-start w-[50px]">Status</p>
            <p className="text-md text-gray-700 flex-grow text-center">Descrição</p>
        ...
        <div className="flex flex-wrap flex-col mt-3 flex-1 gap-3">
          {order?.orderStatus?.map((history, index) => (
            <div key={index} className="flex flex-col flex-1">
              <div className="flex flex-row justify-between">
                <p className="text-xs text-gray-500 text-start w-[50px]">{t(history.status)}</p>
                <p className="text-xs text-gray-500 flex-grow text-center">{history.description}</p>
