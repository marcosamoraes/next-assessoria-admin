...
const usePartnerStoreColumns = (onDelete: (id: number) => void, onStatusToggle: (id: number) => void) => {
  const { user } = useAuth()
    ...
    () => {
      const baseColumns: any = [
        ...
        },
        {
          id: 'city',
          name: 'Cidade/UF',
          selector: (row: any) => `${row.city} - ${row.state}`,
          sortable: true,
        },
        {
          id: 'phone',
            ...
            cell: (row: any) => (
              user?.role !== UserRoleEnum.CONSULTANT && (
                <div className="flex gap-2">
                  <Link href={`/dashboard/lojas-parceiras/editar/${row.id}?${searchParams.toString()}`} as={`/dashboard/lojas-parceiras/editar/${row.id}`}>
                    <TableEditButton />
                  </Link>
...
export default usePartnerStoreColumns
