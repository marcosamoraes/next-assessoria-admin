...
export default function ToggleButton({ name, defaultChecked, value, onChange }: ToggleButtonProps) {
  return (
    <label className="relative inline-flex items-center cursor-pointer">
      <input type="checkbox" value={value} name={name} className="sr-only peer" defaultChecked={defaultChecked} onChange={onChange} />
      <div
        className="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300
              ...
              after:w-5 after:transition-all peer-checked:bg-primary"
      ></div>
    </label>
  )
}
